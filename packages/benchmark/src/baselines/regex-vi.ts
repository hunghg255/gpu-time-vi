...
const parts: Record<string, number> = { sáng: 0, trưa: 12, chiều: 12, tối: 12, đêm: 12 };
...
export function parse(input: string) {
  ...
  if (!date)
    for (const [index, name] of weekdays.entries())
      if (text.includes(name)) {
        ...
        date = new Date(base.getTime() + ((index - current + 7) % 7) * 86_400_000);
        break;
      }
  const numeric = /(\d{1,2})\/(\d{1,2})(?:\/(\d{4}))?/.exec(text);
  if (!date && numeric) {
    const year = numeric[3] ? Number(numeric[3]) : 2026;
    date = new Date(Date.UTC(year, Number(numeric[2]) - 1, Number(numeric[1]), -7));
  }
  const clock = /(\d{1,2})\s*(?:h|g|giờ|:)\s*(\d{2})?\s*(sáng|trưa|chiều|tối|đêm|am|pm)?/.exec(text);
  ...
  if (clock) {
    ...
    const part = clock[3];
    if (part && hour < 12 && (parts[part] ?? (part === "pm" ? 12 : 0))) hour += 12;
    if (part === "đêm" && hour === 12) hour = 0;
  }
  return {
    occurrences: [{ start: iso(date ?? base, hour, minute), allDay: hour === undefined }],
  };
}
